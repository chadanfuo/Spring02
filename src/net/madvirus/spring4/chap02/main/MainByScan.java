...
public class MainByScan {
	...
	private static void useXmlWithScan(){
		//GenericXmlApplicationContext ctx=new GenericXmlApplicationContext("classpath:config-with-scan.xml");
		AnnotationConfigApplicationContext ctx= new AnnotationConfigApplicationContext(ConfigScan.class);
		useContext(ctx);
		ctx.close();
	}
